...
let handler = async (m, { args, conn, usedPrefix, __dirname }) => {
    ...
    if (user.health >= 100) return conn.sendButton(m.chat, `*––––『 FULL HEALTH 』––––*`, `ʏᴏᴜʀ ❤️health ɪs ғᴜʟʟ﹗`.trim(), './media/fullhealth.jpg',
[
[`ᴏᴋ 👌`, `${usedPrefix}ok`]
], m, { asLocation: true })
...
${usedPrefix}transfer potion ${count - user.potion} @${conn.getName(m.sender)}`]
    ...
    conn.sendButton(m.chat, `*––––『 FULL HEALTH 』––––*`, `sᴜᴄᴄᴇssғᴜʟʟʏ *${count} 🥤ᴩᴏᴛɪᴏɴ* ᴜsᴇ ᴛᴏ ʀᴇᴄᴏᴠᴇʀ ʜᴇᴀʟᴛʜ.`, './media/healthrecover.jpg',
[
[`ᴏᴋ 👌`, `${usedPrefix}ok`]
], m, { asLocation: true })
}
